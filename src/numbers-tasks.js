...
function toNumber(value, def) {
  return Number.isInteger(parseInt(value, 10)) ? parseInt(value, 10) : def;
}
...
function getSumOfDigits(num) {
  return num
    .toString()
    .split('')
    .reduce((res, item) => res + parseInt(item, 10), 0);
}
...
function getSumOfNumbers(...numbers) {
  return numbers.reduce((res, item) => res + item, 0).toFixed(1);
}
